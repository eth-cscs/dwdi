...
timestampE_=dt.datetime(2025,2,26,0,0,0)-delta ##INPUT THE DAY OF YOUR JOB
...
def scroll_search(index_name, base_url,timestamp1,timestamp2,user,passw,scroll_time):
  query={
    "bool": {
       "must":{"match":{ "Entry.Label":"clariden"}}, 
      "filter": [
        {
          "range": {
            "@timestamp": {
...
df_data_def.to_csv('table_nid_xanme_extended_clariden.csv',header=True,index=True)
print(df_data_def)