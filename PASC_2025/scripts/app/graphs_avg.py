...
def apply_data_filters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_filtered['power'] = df_filtered['average_energy_per_node'] / (df_filtered['elapsed'] * df_filtered['total_nodes'])
    df_filtered = df_filtered[df_filtered['power'] <= 2700]
    # skip NaN values
    df_filtered['average_energy_per_node'] = df_filtered['average_energy_per_node'].where(df_filtered['average_energy_per_node'].notna(), None)
...
def create_pie_chart(df: pd.DataFrame, cluster: str, quantity: str) -> None:
    ...
    Args:
        ...
        quantity: Quantity to plot ('node_hours' or 'average_energy_per_node')
    """
    ...
    if quantity == 'node_hours':
        ...
        df_pie = df[['account_', 'average_energy_per_node']]
        df_pie_real = df[['account', 'average_energy_per_node']]
        unit = 'GJ'
...
def create_histogram(df: pd.DataFrame, cluster: str, postfix: str = '') -> None:
    ...
    """
    plt.hist(df['average_energy_per_node'], color='blue', edgecolor='black', bins=100)
...
def create_heatmap(df: pd.DataFrame, cluster: str, threshold: int, cmap: str = 'RdYlBu_r') -> None:
    ...
    sample_data = df[df['average_energy_per_node'] < threshold].copy()

    # Create energy bins
    energy_max = int(sample_data['average_energy_per_node'].max())
    delta = energy_max // 20
    ...
    # Bin the energy data
    sample_data['range_energy'] = pd.cut(sample_data['average_energy_per_node'], bins=bins, labels=labels, include_lowest=True)
    ...
    ax=sns.heatmap(heatmap_matrix, cmap=custom_cmap, linewidths=0.5, annot=True, fmt=".0f",annot_kws={"size": 16})
...
def barcharts_nh(df:pd.DataFrame,cluster):
    ...
    cbar.set_label('Total Node Hours', rotation=270, labelpad=15)
    # Axis labels
    plt.title('Distribution of jobs by resources')
    ...
    plt.tight_layout()
    plt.yscale('log')
    plt.savefig(f"png/job_distribution_bar_{cluster}.png", bbox_inches='tight', dpi=200)
    plt.show()
...
def barcharts_en(df:pd.DataFrame,cluster):
    ...
    # Group by bucket
    grouped = df.groupby('bucket').agg(
        job_count=('average_energy_per_node', 'count'),
        total_avg_energy=('average_energy_per_node', 'sum')
    ).reindex(['1', '2 → 4', '4 → 8','8 → 16','16 → 32','32 → 64','64 → 128','128 → 256','256 → 512','512 → 1024','1024 → 2048','2048 → 4096'])



    # Normalize node_hours to 0-1 for colormap
    norm = mcolors.Normalize(vmin=grouped['total_avg_energy'].min(), vmax=grouped['total_avg_energy'].max())
    cmap = cm.Reds
    colors = cmap(norm(grouped['total_avg_energy']))
    ...
    sm.set_array([])  # only needed for older matplotlib
    cbar = plt.colorbar(sm, ax=ax)
    cbar.set_label('Average Energy Sum', rotation=270, labelpad=15)
    # Axis labels
    plt.title('Distribution of jobs by resources')
    plt.ylabel('Job count')
    ax.set_xticklabels(grouped.index, rotation=45, ha='right')
...
def main():
    ...
    for cluster_name, df in cluster_dfs.items():
        ...
        if args.pie or args.all:
            create_pie_chart(df, cluster=cluster_name, quantity='average_energy_per_node')
    ...
    if not args.cluster:
        ...
        if args.pie or args.all:
            create_pie_chart(df_all, cluster='big 3', quantity='average_energy_per_node')
